...
from data.othello import permit, start_hands, OthelloBoardState, DLCBoardState
...
if args.twolayer:
    folder_name = folder_name + f"_tl_dlc{args.mid_dim}"  # tl for probes without batchnorm #
if args.random:
...
act_container = [] #element size: [512, ]
property_container = [] # element size: [64, ]
# is_first = True
for x, y in tqdm(loader, total=len(loader)):
    ...
    # if is_first: #
    #     print("\n\ny: ", len(property_container[0]), "\n\n")
    #     is_first = False
    #     break

# pad the properties (y) to length of 144:
dlc_property_container = []
for y in property_container:
    pad = [1.0]*4
    new_y = y[:8] + pad + y[8:16] + pad + y[16:24] + pad + y[24:32] + pad + y[32:40] + pad + y[40:48] + pad + y[48:56] + pad + y[56:] + pad + [1.0]*48
    dlc_property_container.append(new_y)
    
age_container = [] #element size: [512, ]
# is_first = True
for x, y in tqdm(loader, total=len(loader)):
    tbf = [train_dataset.itos[_] for _ in x.tolist()[0]]
    valid_until = tbf.index(-100) if -100 in tbf else 999
    a = OthelloBoardState() #
    ages = a.get_gt(tbf[:valid_until], "get_age")  # [block_size=512, ]
...
if args.twolayer:
    probe = BatteryProbeClassificationTwoLayer(device, probe_class=probe_class, num_task=144, mid_dim=args.mid_dim) #
else:
...
probing_dataset = ProbingDataset(act_container, dlc_property_container, age_container) #
...
tconf = TrainerConfig(
    ...
    ckpt_path=os.path.join("./ckpts_/", folder_name, f"layer{args.layer}")
)
